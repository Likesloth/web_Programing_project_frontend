<template>
  <q-page class="flex flex-center">
    <q-card dark bordered class="bg-grey-2 my-card q-px-md p-py-md">
      ...
      <q-card-section>
        <q-form @submit.prevent="onSubmit" @reset="onReset" class="q-gutter-md" ref="registerForm">
          <div>
            <q-input v-model="fullname" type="text" label="Your Fullname" lazy-rules :rules="[requiredValidate]" />
          </div>

          <div>
            <q-input v-model="username" type="text" label="Your Username" lazy-rules :rules="[requiredValidate]" />
            <text-caption style="font-size: 0.9em" v-if="usernameCaption.showStatus" :class="[
          ...
          <div>
            <q-input v-model="password" :type="isPwd ? 'password' : 'text'" label="Your Password" lazy-rules :rules="[
              (val) =>
  ...
  />
    ...
    <dialog-component v-model="showDialog" :propDialog="dialog"></dialog-component>
  </q-page>
</template>
